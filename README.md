...
> 系统采用六阶段 Pipeline 架构：Plan Agent 分析 Swagger 文档生成测试计划 → Generator Agent 按模块生成 YAML 用例 → Reviewer Agent 审核用例质量 → Coverage Analyzer 检测覆盖率 → Feedback Loop 补充缺失用例 → Performance Test 执行压测。整个流程通过 `workflow.py` 编排，一条命令自动执行。
